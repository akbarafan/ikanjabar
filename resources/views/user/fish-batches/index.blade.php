            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['total_transferred']) }}</p>
                        <p class="text-xs text-gray-500">Ekor</p>
                    </div>
                </div>
            </div>
                    ...
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Batch
                                Info</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kolam
                                & Jenis</th>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4">
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-500">Mulai:
                                            {{ \Carbon\Carbon::parse($batch->date_start)->format('d M Y') }}</div>
                                        @if ($batch->notes)
                                            <div class="text-xs text-gray-400 mt-1">{{ Str::limit($batch->notes, 30) }}
                                            </div>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">
                                        <span class="font-medium text-lg">{{ number_format($batch->current_stock) }}</span>
                                        <span class="text-gray-500">/ {{ number_format($batch->initial_count) }}</span>
                                    </div>
                                    <div class="text-xs text-gray-500">Saat ini / Awal</div>
                                    @if ($batch->sold > 0 || $batch->mortality > 0)
                                        <div class="text-xs text-gray-400 mt-1">
                                            @if ($batch->sold > 0)
                                                Terjual: {{ number_format($batch->sold) }}
                                            @endif
                                            @if ($batch->mortality > 0)
                                                @if ($batch->sold > 0)
                                                    ,
                                                @endif Mati: {{ number_format($batch->mortality) }}
                                            @endif
                                        </div>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    @if ($batch->transferred_in > 0 || $batch->transferred_out > 0)
                                        <div class="space-y-1">
                                            @if ($batch->transferred_in > 0)
                                                <div class="flex items-center text-xs">
                                                    <i class="fas fa-arrow-down text-green-600 mr-1"></i>
                                                    <span
                                                        class="text-green-600">+{{ number_format($batch->transferred_in) }}</span>
                                                </div>
                                            @endif
                                            @if ($batch->transferred_out > 0)
                                                <div class="flex items-center text-xs">
                                                    ...
                                                    <span
                                                        class="text-red-600">-{{ number_format($batch->transferred_out) }}</span>
                                                </div>
                                            @endif
                                        </div>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-xs text-gray-500 mb-2">{{ $batch->age_weeks }} minggu</div>
                                    <span
                                ...
                                <td class="px-6 py-4 text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        ...
                                        <button onclick="deleteBatch({{ $batch->id }}, 'Batch #{{ $batch->id }}')"
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 mb-4">Mulai dengan menambahkan batch ikan pertama untuk
                                            cabang ini.</p>
                                        <button onclick="openAddModal()"
                                            class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors">
                                            <i class="fas fa-plus mr-2"></i>
            ...
            <div class="mt-3">
                ...
                <form id="batchForm" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="pond_id" name="pond_id" required
                                ...
                                @foreach ($ponds as $pond)
                                    <option value="{{ $pond->id }}">{{ $pond->name }} ({{ $pond->code }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            <label for="fish_type_id" class="block text-sm font-medium text-gray-700 mb-1">Jenis Ikan
                                *</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label for="date_start" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Mulai
                                *</label>
                        ...
                        <div>
                            <label for="initial_count" class="block text-sm font-medium text-gray-700 mb-1">Jumlah Awal
                                *</label>
